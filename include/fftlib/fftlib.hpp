...
#include "fftlib_ext_plan.hpp"
	...
	#include "fftlib_composed_fft.hpp"
#endif
...
namespace fftlib
{
	...
	{	
		template<backend BB, transformation TT, std::int32_t DD>
		friend class fft;

		#if defined(FFTLIB_COMPOSED_FFT)
			template<backend BB, transformation TT>
			friend class fftlib_internal::composed_fft;
		#endif
	...
	public:
		...
		static void execute_plan(const typename fftlib_internal::ext_plan& p, typename trafo<B, T>::in_t* in = NULL, typename trafo<B, T>::out_t* out = NULL);
	...
	private:
		...
		#if defined(FFTLIB_COMPOSED_FFT)
			void create_composed_fft(typename fftlib_internal::ext_plan& p, const typename data<B, INT>::type_t* n, const typename data<B, INT>::type_t howmany, typename trafo<B, T>::in_t* in, const typename data<B, INT>::type_t* inembed, const typename data<B, INT>::type_t istride, const typename data<B, INT>::type_t idist, typename trafo<B, T>::out_t* out, const typename data<B, INT>::type_t* onembed, const typename data<B, INT>::type_t ostride, const typename data<B, INT>::type_t odist, const typename data<B, INT>::type_t sign, const typename data<B, UINT>::type_t flags);
		#endif
...
#if defined(FFTLIB_COMPOSED_FFT)
	#include "fftlib_composed_fft_implementation.hpp"
#endif
