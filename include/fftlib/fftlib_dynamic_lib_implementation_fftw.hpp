...
namespace fftlib_internal
{
	...
	{	
		...
		(*(xxx_plan_with_nthreads))(this->nthreads[omp_get_thread_num()]);
	}
...
#undef  FFTLIB_FUNCTION_SIGNATURE
	...
	{		
		...
		#if defined(FFTLIB_USE_MKL)
			if (!omp_in_parallel())		
				mkl_domain_set_num_threads(get_nthreads(), MKL_DOMAIN_FFT);
		#endif
	...
	template<>
        void dynamic_lib<FFTW>::execute_plan<C2C_64>(const typename trafo<FFTW, C2C_64>::plan_t p, typename trafo<FFTW, C2C_64>::in_t* in, typename trafo<FFTW, C2C_64>::out_t* out, const typename data<FFTW, INT>::type_t nthreads) const
	{
		// bypass internal logic.
		#if defined(FFTLIB_BYPASS)
	...
	template<>
        void dynamic_lib<FFTW>::execute_plan<R2C_64>(const typename trafo<FFTW, R2C_64>::plan_t p, typename trafo<FFTW, R2C_64>::in_t* in, typename trafo<FFTW, R2C_64>::out_t* out, const typename data<FFTW, INT>::type_t nthreads) const
	{
		// bypass internal logic.
		#if defined(FFTLIB_BYPASS)
	...
	template<>
	void dynamic_lib<FFTW>::execute_plan<C2R_64>(const typename trafo<FFTW, C2R_64>::plan_t p, typename trafo<FFTW, C2R_64>::in_t* in, typename trafo<FFTW, C2R_64>::out_t* out, const typename data<FFTW, INT>::type_t nthreads) const
	{
		// bypass internal logic.
		#if defined(FFTLIB_BYPASS)
