...
namespace fftlib_internal
{
	...
	public:
		...
		template<transformation T>
		void execute_plan(const typename trafo<FFTW, T>::plan_t p, typename trafo<FFTW, T>::in_t* in, typename trafo<FFTW, T>::out_t* out, const typename data<FFTW, INT>::type_t nthreads = -1) const;
