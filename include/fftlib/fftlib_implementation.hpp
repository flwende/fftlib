...
namespace fftlib
{
	...
	fft<B, T, D>::~fft()
	{
		...
		#if defined(FFTLIB_COMPOSED_FFT)
			fftlib_internal::composed_fft<B, T>::cleanup();
		#endif
	}
	...
	typename trafo<B, T>::plan_t fft<B, T, D>::create_plan(const typename data<B, INT>::type_t* n, const typename data<B, INT>::type_t howmany, typename trafo<B, T>::in_t* in, const typename data<B, INT>::type_t* inembed, const typename data<B, INT>::type_t istride, const typename data<B, INT>::type_t idist, typename trafo<B, T>::out_t* out, const typename data<B, INT>::type_t* onembed, const typename data<B, INT>::type_t ostride, const typename data<B, INT>::type_t odist, const typename data<B, INT>::type_t sign, const typename data<B, UINT>::type_t flags)
	{
		...
		p->nthreads = dl.get_nthreads();
		p->composed_fft = false;

		#if defined(FFTLIB_COMPOSED_FFT)
			if (T == C2C_64 && D == 3 && howmany == 1 && (istride + ostride) == 2)
				{
					// use composed fft
					p->composed_fft = true;
					fftlib_internal::composed_fft<FFTW, C2C_64>::create<3>((*p), n, reinterpret_cast<typename trafo<B, C2C_64>::in_t*>(in), reinterpret_cast<typename trafo<B, C2C_64>::out_t*>(out), sign, flags);
				}
			else
		#endif
		{
			// create a plan configuration object.
			configuration c = configuration(n, howmany, in, inembed, istride, idist, out, onembed, ostride, odist, sign, flags, p->nthreads);
	...
	void fft<B, T, D>::execute_plan(const typename trafo<B, T>::plan_t p, typename trafo<B, T>::in_t* in, typename trafo<B, T>::out_t* out)
	{
		...
		#endif
				{
					FFTLIB_INFO("fft execute_plan (thread=" << omp_get_thread_num() << ") plan was created by fftlib");
					...
					fftlib_internal::ext_plan* pp = reinterpret_cast<fftlib_internal::ext_plan*>(p);
						...
						if (pp->composed_fft)
							{
								if (in == NULL && out == NULL)
									fftlib_internal::composed_fft<B, C2C_64>::execute((*pp), reinterpret_cast<typename trafo<B, C2C_64>::in_t*>(pp->in), reinterpret_cast<typename trafo<B, C2C_64>::out_t*>(pp->out));
								else
									fftlib_internal::composed_fft<B, T>::execute((*pp), in, out);
							}
						else
					#endif
					{
						if (in == NULL && out == NULL)
							switch (pp->t)
								{
								case C2C_64:
									dl.template execute_plan<C2C_64>(*reinterpret_cast<const typename trafo<B, C2C_64>::plan_t*>(pp->p_1), reinterpret_cast<typename trafo<B, C2C_64>::in_t*>(pp->in), reinterpret_cast<typename trafo<B, C2C_64>::out_t*>(pp->out), pp->nthreads);
									break;
									
								case R2C_64:
									dl.template execute_plan<R2C_64>(*reinterpret_cast<const typename trafo<B, R2C_64>::plan_t*>(pp->p_1), reinterpret_cast<typename trafo<B, R2C_64>::in_t*>(pp->in), reinterpret_cast<typename trafo<B, R2C_64>::out_t*>(pp->out), pp->nthreads);
									break;
									
								case C2R_64:
									dl.template execute_plan<C2R_64>(*reinterpret_cast<const typename trafo<B, C2R_64>::plan_t*>(pp->p_1), reinterpret_cast<typename trafo<B, C2R_64>::in_t*>(pp->in), reinterpret_cast<typename trafo<B, C2R_64>::out_t*>(pp->out), pp->nthreads);
									break;
							...
							dl.template execute_plan<T>(*reinterpret_cast<const typename trafo<B, T>::plan_t*>(pp->p_1), in, out, pp->nthreads);
					}
				}
		#if !defined(FFTLIB_UNSAFE_OPT)
			else
